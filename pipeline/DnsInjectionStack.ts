...
export class DnsInjectionStack extends Stack {
  constructor(scope: Construct, id: string, props: StackProps) {
    ...
    const responseLambda = new Function(this, "response", {
    ...
    api.root.addMethod("POST", new LambdaIntegration(responseLambda));

    const requestLambda = new Function(this, "request", {
      runtime: Runtime.NODEJS_20_X,
      handler: "index.handler",
      code: Code.fromInline(
        `exports.handler = async (event) => ({
              statusCode: 200,
              body: await fetch('https://example.test.com', { method: 'POST' }).then(data => data.text())
        })`,
      ),
    });

    api.root.addMethod("GET", new LambdaIntegration(requestLambda));
  }
}
